...
hmm_dir = Path(f'{base_dir}/tms_mdd/57patients_newmodels_giles_plots')
# where are the source recos stored? this is the patient ID base
prep_dir = Path(f'{base_dir}/tms_mdd/prepared_data_clean_giles_newmodel')

#save_summary_stats_df(patients_clean=True, n_states=n_states)
...
def match_hads_to_eeg_session():
    ...
    clinical_dir = f'{base_dir}/LabData/Lab_LucaC/A_QNC_Databank/Participants_Clinical_TMS_Data'
    ...
    eeg_dir = Path(f"{base_dir}/LabData/Lab_LucaC/A_QNC_ANT_Data/TMS_MDD_EEG_data")
    ...
    matched_df.to_csv(f'{hmm_dir}/hads_matched_to_eeg_sess_upto209.csv')
...
def save_summary_stats_df(n_sessions: int = 6, n_states: int = n_states, patients_clean: bool = False):
    ...
    df = pd.read_csv(f'{base_dir}/tms_mdd/allquestionnaires_demo_combined.csv')
